...
<body class="hold-transition login-page" style=" background-image: linear-gradient(to bottom right, #FF8000, yellow);">
  <div class="login-box">
    ...
    <!-- /.login-logo -->
  <?php
  if (!empty($this->session->flashdata('change_pass_error'))) {
      echo "<div class='alert alert-danger' id='error'>".$this->session->flashdata('change_pass_error')."</div>";
    }
 if (!empty($this->session->flashdata('token_error'))) {
      echo "<div class='alert alert-danger' id='error'>".$this->session->flashdata('token_error')."</div>";
    }
     if (!empty($this->session->flashdata('token_error1'))) {
      echo "<div class='alert alert-danger' id='error'>".$this->session->flashdata('token_error1')."</div>";
    }
    ?>
      ...
      <div class="card-body login-card-body bg-light">
        ...
        <form action="<?php echo base_url().'change_password'?>" method="post">
           <input type="hidden" name="token" value="<?php if(isset($_GET['token'])){echo $_GET['token'];} ?>">
            <input type="hidden" name="mail" value="<?php if(isset($_GET['mail'])){echo $_GET['mail'];} ?>">
